...
func (input Input) Lines() (out []string) {
	for _, it := range common.Lines(input.Text()) {
...
func (input Input) Text() string {
	path := input.Path()
	data := common.Try(os.ReadFile(path))
	return string(data)
}
...
func (input Input) Json(out any) {
	path := input.Path()
	data := common.Try(os.ReadFile(path))
	common.NoError(json.Unmarshal(data, out), "failed to parse JSON")
}
...
func (runner Runner) Run() (out []RunOutput) {
	failed := 0
	for _, it := range common.Glob(runner.rootDir, runner.glob) {
		run := RunOutput{
			t:     runner.t,
			root:  runner.rootDir,
...
func (run *RunOutput) runSingle(runner TestRunner) {
	...
	expectText := common.ReadText(run.outFile())
...
func (run *RunOutput) checkResult() {
	...
	if run.Success && (!hasOutFiles || Rewrite) && (hasActualOutput || hasJsonOutput) {
		if hasActualOutput {
			common.WriteText(run.outFile(), run.Output.StdOut)
		}
		if hasJsonOutput {
			common.WriteJson(run.outJson(), run.Output.Data)
		}
	}
...
func (run RunOutput) OutputDetails() {
	...
	if output.StdErr != "" {
		outputBanner()
		run.output("\n  - Error output:\n\n")
		for _, it := range common.TrimLines(common.Lines(output.StdErr)) {
			run.output("      %s\n", it)
		}
	}
